...
- Active branch: `codex/embed-artifacts`
- Active PR lane: PR6 Pre-PR review gate rerun after plural session-token redaction fix
- Sibling repos: off-limits unless a future task explicitly names them
- Current task: PR6 `embed` implementation; preserve current CLI/API/MCP contracts and merged `ingest`/`chunk` behavior
...
| PR6 | `codex/embed-artifacts` | In progress | `embed` and cache/profile behavior. |
...
- Required next: rerun the Pre-PR Codex Review Gate on `codex/embed-artifacts`; if clean, commit, push, open the PR, then check GitHub checks and remote review/Copilot comments.
